...
class Annotator:
    ...
    def cv_text(self,xy,text, txt_color=(255, 255, 255)):
        tf =10  # font thickness
        cv2.putText(self.im,
                    text, xy,
                    0,
                    self.lw *2,
                    ...
                    lineType=cv2.LINE_AA)
    def mosaic(self,box):
        x = int(box[0])   # 剪裁區域左上 x 座標
        y =  int(box[1] )   # 剪裁區域左上 y 座標
        cw =  int(box[2]-box[0])  # 剪裁區域寬度
        ch =  int(box[3]-box[1]) # 剪裁區域高度
        mosaic = self.im[y:y+ch, x:x+cw]   # 取得剪裁區域
        level = 15         # 馬賽克程度
        ...
        if(w!=0 and h!=0):
            ...
            self.im[y:y+ch, x:x+cw] = mosaic   # 將圖片的剪裁區域，換成馬賽克的圖
